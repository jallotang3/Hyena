...
class NoticeUseCase {
  NoticeUseCase({required PanelAdapter adapter, required PanelSite site})
      : _adapter = adapter,
        _site = site;
  ...
  Future<Result<List<Notice>>> fetchNotices({int page = 1}) async {
    try {
      final auth = await SecureStorage.instance.readAuthContext();
      if (auth == null) return Failure(const AuthException('未登录'));
      final notices = await _adapter.fetchNotices(_site, auth, page: page);
